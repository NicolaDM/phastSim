...
for t in $TIMES; do
    for s in $SEEDS; do
        ...
        echo experiment $t $s "${PHASTSIM_OPTIONS[$i]}" $seqgenOptions
        
        python ../scripts/compareSimulators.py --nLeaves $t $seqgenOptions --phastSim --createFasta --noHierarchy --seed $s --reference ./sequence.fasta | tail -1 >> indel_data.txt
