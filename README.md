...
phastSim --path simulation_output/ --seed 7 --createFasta \
...
```sh
mkdir simulation_output
python bin/phastSimulate --path simulation_output/ --seed 7 --createFasta \
                         --createNewick --createPhylip --treeFile phastSim/example/rob-11-7-20.newick \
                         --scale 3.0 --invariable 0.1 --alpha 1.0 --omegaAlpha 1.0 \
                         --hyperMutProbs 0.01 0.01 --hyperMutRates 20.0 200.0 --codon \
                         --reference phastSim/example/MN908947.3.fasta
```
which uses an example tree included here, specified by `--treeFile phastSim/example/rob-11-7-20.newick`, and a reference SARS-CoV-2 genome in FASTA format, provided here and specified by `--reference phastSim/example/MN908947.3.fasta`.
...
For small phylogenies (<1,000 tips) and nucleotide models, the software seq-gen by Rambaut and Grassly will typically be more efficient (due to fixed costs, for example associated with loading Python packages).
This script does not yet allow simulations of indels - for this you will probably need INDELible by Fletcher and Yang.
...
A .info text file is also created, which describes the rate categories for each site of the genome.
