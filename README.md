...
--seed 10 --createInfo --createFasta --createNewick --createMAT --createPhylip --verbose --noHierarchy
```
...
The simulator does not use hierarchical mode, so should be faster. 
...
- phastSim assumes that branch lengths are given as substitutions per site. If
they are given as substitutions per nucleotide, but you are using a codon model, 
you should set --scale 0.33333333 to correctly adjust for this. 
